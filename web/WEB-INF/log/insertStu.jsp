                            ...
                            <div role="tabpanel" class="tab-pane" id="advanced"
                                ...
                                <div class="col-sm-5" style="margin-top: 20px">
                                    ...
                                    <ul class="list-group" id="fixedlist">
                                        <li class="list-group-item active">学生固定结构</li>
                                        <li class="list-group-item fixedappend">123123</li>
                                    </ul>
                                    <ul class="list-group" id="extensiblelist">
                                        <li class="list-group-item active">学生可变结构</li>
                                    </ul>
...
<script>
    ...
    $("#fileAnalysebtn").click(function () {
        ...
        $.post("/excle/getItems", function (data) {
                    ...
                    /*option添加空*/
                    $.post("/insert/getFixed", function (data) {/*获取固定表结构*/
                        if (data != null) {
                    ...
                    $.post("/insert/getExtensible", function (data) {/*获取扩展表表结构*/
                        if (data != null) {
                            for (var token in data) {
                                $(".Mappingselect").append("<option  value=" + data[token].jlb_lm + ">" + data[token].jlb_lm + data[token].jlb_bz + data[token].jlb_type + "</option>")
                            }
    ...
    $("#btn_getbjg").click(function () {
        $(".fixedappend").remove();
        $.post("/insert/getFixed", function (data) {
            if (data != null) {
                for (var token in data) {
        ...
        $.post("/insert/getExtensible", function (data) {
            $(".extensibleappend").remove();
            if (data != null) {
                for (var token in data) {
                    $("#extensiblelist").append("<li class='list-group-item fixedappend'> <h5 class='list-group-item-heading'> 字段名：" + data[token].jlb_lm + "  备注:" + data[token].jlb_bz + "<small>  类型" + data[token].jlb_type + "</small></h5> </li>")
                }
            }
        });
    })
    ...
    $("#btn_delsubmti").click(function () {
        ...
        $.post("/insert/deltekzxxb", {colname: $("#input_del_bm").val()}, function (data) {
            if (data == "success") {
                alert("修改成功!")
            }
            else {
    ...
    $("#btn_addsubmti").click(function () {
        alert($("#input_type").val())
        $.post("/insert/addkzxxb", {
            newcol: $("#input_bm").val(),
            type: $("#input_type").val(),
